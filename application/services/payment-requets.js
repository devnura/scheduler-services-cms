...
exports.generatePaymentRequest = async () => {
  ...
  const service_name = "generate payment request"
  try {

    winston.logger.info(
      `EXEC ID : ${exec_id} | SERVICE : ${service_name} | STARTING SCHEDULER GENERATE PAYMENT REQUEST`
    );

    const parameter = await knex('public.t_m_global_parameter AS tmgp').select([
        "tmgp.c_value"
      ])
      .where('c_code', '=', "SCEDULER_URL_POSTING_PAYMENT")
      .where('c_status', '=', "A")
      .first()

    if(!parameter) throw new Error("Failed getting parameter SCEDULER_URL_POSTING_PAYMENT !");
