...
app.listen(process.env.SERVICE_PORT, () => {
  winston.logger.info(
    `Server is running on environment: ${process.env.SERVICE_ENV.toUpperCase()} port: ${
...
cron.schedule('0 * * * *', async () => {
  console.log('scheduler service is runnig');
});
...
cron.schedule('30 7 * * *', async () => {
  try {
    const service = await retryPaymentRequest.service();
    console.log(service);
  } catch (error) {
    console.log(error);
  }
